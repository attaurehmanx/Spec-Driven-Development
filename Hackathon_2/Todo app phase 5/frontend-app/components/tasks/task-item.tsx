...
const getPriorityGradient = (priority?: PriorityLevel, completed?: boolean) => {
  if (completed) {
...
// Get recurring icon and label
const getRecurringInfo = (recurring: RecurringPattern) => {
  switch (recurring) {
    case 'daily':
      return { icon: '📅', label: 'Daily' };
    case 'weekly':
      return { icon: '📆', label: 'Weekly' };
    case 'monthly':
      return { icon: '🗓️', label: 'Monthly' };
    default:
      return null;
  }
};
...
export const TaskItem: React.FC<TaskItemProps> = ({ task, onToggle, onDelete }) => {
  ...
  const gradientClass = getPriorityGradient(task.priority, task.completed);
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground/80">
              {/* Priority Badge */}
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${getPriorityColor(task.priority)} text-white shadow-sm`}>
                <span>{getPriorityIcon(task.priority)}</span>
                <span>{task.priority.charAt(0).toUpperCase() + task.priority.slice(1)}</span>
              </span>

              {/* Recurring Badge */}
              {task.recurring && task.recurring !== 'none' && getRecurringInfo(task.recurring) && (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-purple-500 to-indigo-500 text-white shadow-sm">
                  <span>{getRecurringInfo(task.recurring)?.icon}</span>
                  <span>{getRecurringInfo(task.recurring)?.label}</span>
                </span>
              )}
              ...
              {task.due_date && (
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${
                  isOverdue
                    ? 'bg-gradient-to-r from-red-500 to-orange-500 text-white shadow-sm animate-pulse'
