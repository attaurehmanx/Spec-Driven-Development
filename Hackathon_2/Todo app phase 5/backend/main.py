...
async def lifespan(app: FastAPI):
    ...
    # Initialize APScheduler for reminder jobs (Phase 6: Due Dates and Reminders)
    try:
        ...
        scheduler.start()
        logger.info("APScheduler started - reminder job scheduled to run every 5 minutes")
    except Exception as e:
        logger.error(f"Failed to start APScheduler: {e}")
        # Don't fail startup if scheduler fails - reminders are optional
    ...
    # Shutdown reminder publisher
    try:
        from events.reminder_publisher import shutdown_publisher
        await shutdown_publisher()
        logger.info("Reminder publisher shut down")
    except Exception as e:
        logger.error(f"Error shutting down reminder publisher: {e}")
...
# For local development, allow all origins. For production, restrict to specific domains.
app.add_middleware(
    CORSMiddleware,
