...
## Current Project: Multi-User Task Management Web Application
...
Use Spec-Kit Plus workflow: Write spec → Generate plan → Break into tasks → Implement via Claude Code. No manual coding allowed. Review process, prompts, and iterations to judge each phase.

### Core Requirements
- Implement 5 Basic Level features as a web application (to be specified)
...
### Security Architecture: Better Auth + FastAPI Integration
...
- All API endpoints must verify JWT tokens
- All data queries must filter by authenticated user ID
- User ID in URL must match authenticated user from JWT token
